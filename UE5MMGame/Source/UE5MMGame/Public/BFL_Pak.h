 ...
 */
UCLASS()
class UE5MMGAME_API UBFL_Pak : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()
	
public:

	//property bool TreatWChar_tAsBuiltInType { bool get(); void set(bool value); };

	UFUNCTION(BlueprintCallable, Category = "PAK")
	static void MountAndRegisterPak(FString PakFilePath, bool& bIsMountSuccessful);
	
	UFUNCTION(BlueprintCallable, Category = "PAK")
	static void UnmountAndUnregisterPak(FString PakFilePath, bool& bIsMountSuccessful);
	...
	UFUNCTION(BlueprintCallable, BlueprintPure, Category = "PAK")
	static TArray<FString> GetModDirectories(const FString& modsPath);

	UFUNCTION(BlueprintCallable, BlueprintPure, Category = "PAK")
	static FString GetModFileName(const FString modPath);
};
